...
internal sealed class McpServer(FindFastService service, TextReader input, TextWriter output, TextWriter error)
{
    ...
    private async Task<JsonNode> CallToolAsync(JsonObject parameters, CancellationToken cancellationToken)
    {
        ...
        object result = name switch
        {
            ...
            "root_update" => await service.RootUpdateAsync(RequiredString(args, "root_id"), new RootUpdateOptions {
                Include = Strings(args, "include"), Exclude = Strings(args, "exclude"), Extensions = Strings(args, "extensions"),
                RespectGitignore = args["respect_gitignore"] is null ? null : Bool(args, "respect_gitignore", true) }, cancellationToken),
...
internal static class ToolDefinitions
{
    ...
    [
        ...
        Tool("root_add", "Register and index a local directory.", new { path = S("Absolute directory path"), name = S("Friendly name"),
            include = A("Include globs"), exclude = A("Exclude globs"),
            extensions = A("Indexable extensions, such as cs or .cs. Omit for the built-in default set; use ['*'] for every text file"),
            respect_gitignore = B("Respect root .gitignore rules") }, ["path"]),
        Tool("root_update", "Change the filters of a registered root and reconcile its index. Omitted fields are kept.",
            new { root_id = S("Root identifier"), include = A("Include globs"), exclude = A("Exclude globs"),
            extensions = A("Indexable extensions. Empty array restores the default set; ['*'] indexes every text file"),
            respect_gitignore = B("Respect root .gitignore rules") }, ["root_id"]),
