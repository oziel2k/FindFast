...
function Normalize-Extensions($Values) {
    ...
    # '*' opts the root out of extension filtering; an empty list selects the server's default set.
    foreach ($raw in @($Values)) { if($null-eq$raw -or $raw-is[pscustomobject] -or $raw-is[System.Collections.IDictionary]){continue}; $v = ([string]$raw).Trim(); if ($v -eq '*') { $result += '*'; continue }; if (!$v -or $v -match '[\\/\*\?\[\]]') { throw "Extensão inválida: '$raw'" }; $v=$v.TrimStart('.'); if($v -notmatch '^[A-Za-z0-9_-]+$'){throw "Extensão inválida: '$raw'"}; $result += ".$($v.ToLowerInvariant())" }
...
function Register-Client([string]$Client, [string]$ExePath) {
    ...
    # 'claude mcp add' declares -e/--env as variadic; every following non-option token is swallowed by it.
    # The server name must therefore precede the options, and '--' must close the list before the executable,
    # or the name binds to the executable and commandOrUrl is reported missing.
    $addArgs = if($Client -eq 'codex'){@('mcp','add','findfast','--env',"FINDFAST_DATA_DIR=$DataDirectory",'--',$ExePath)}else{@('mcp','add','findfast','--transport','stdio','--scope','user','--env',"FINDFAST_DATA_DIR=$DataDirectory",'--',$ExePath)}
    $added=Invoke-Cli $command.Source $addArgs; if($added.ExitCode -ne 0){Write-Log "Falha registrando ${Client}: $($added.Output)"; return $false}
...
try {
    ...
    $changed=@(); foreach($requested in $roots){$path=Canonical $requested.path; $found=$catalog|Where-Object{(Canonical $_.path)-ieq $path}|Select-Object -First 1; $ext=@(Normalize-Extensions $requested.extensions); if(!$found){$id=([IO.Path]::GetFileName($path)-replace '[^A-Za-z0-9]+','-').Trim('-').ToLowerInvariant();if(!$id){$id='root'};$base=$id;$n=2;while($catalog.root_id -contains $id){$id="$base-$n";$n++};$found=[pscustomobject]@{root_id=$id;name=if($requested.name){$requested.name}else{[IO.Path]::GetFileName($path)};path=$path;type=if(Test-Path (Join-Path $path '.git')){'git_repository'}else{'directory'};include=@(Normalize-StringArray $requested.include);exclude=@(Normalize-StringArray $requested.exclude);extensions=$ext;respect_gitignore=if($null-ne $requested.respect_gitignore){[bool]$requested.respect_gitignore}else{$true};state='stale';version=0;last_updated=$null;last_error='Index is missing or unavailable.';file_count=0};$catalog += $found;$changed += $found.root_id}else{$before=($found|ConvertTo-Json -Depth 8 -Compress);$found.include=@(Normalize-StringArray $requested.include);$found.exclude=@(Normalize-StringArray $requested.exclude);$found.extensions=$ext;$found.respect_gitignore=if($null-ne $requested.respect_gitignore){[bool]$requested.respect_gitignore}else{$true};if(($found|ConvertTo-Json -Depth 8 -Compress)-ne $before){$found.state='stale';$changed += $found.root_id}}}
